...
def _make_hidden_layer(in_dim, out_dim, activation, dropout=None):
    if dropout:
        return nn.Sequential(
            nn.Linear(in_dim, out_dim), activation, nn.Dropout(p=dropout)
        )
...
class MortgageNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = nn.modules.EmbeddingBag(
            self.input_size, self.embedding_size, mode=embedding_bag_mode
        )
        ...
        if len(hidden_dims) > 0:
            ...
            hidden_layers = [
                _make_hidden_layer(
                    dims[i], dims[i + 1], self.activation, self.dropout
                )
