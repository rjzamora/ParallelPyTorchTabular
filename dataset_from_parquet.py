...
def load_tensors_from_parquet(path, target_name="delinquency_12"):
    ...
    if target_name in tbl:
        target = torch.from_numpy(
            tbl.pop(target_name).values.astype(np.float32)
        )
    ...
    def __init__(
        ...
        if shuffle_files:
            self.parquet_files = list(
                np.random.permutation(self.parquet_files)
            )
...
def dataset_from_parquet(root_path, num_samples=None, shuffle_files=False):
    return MortgageParquetDataset(
        root_path, num_samples=num_samples, shuffle_files=shuffle_files
    )
