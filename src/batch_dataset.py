...
class TensorBatchDataset(BatchDataset):
    ...
    def __getitem__(self, item):
        ...
        if idx < self.num_samples and (
            idx + self.batch_size < self.num_samples
            or self.num_samples % self.batch_size == 0
        ):
            return tuple(
                tensor[idx : idx + self.batch_size] for tensor in self.tensors
            )
        elif (
            idx < self.num_samples and idx + self.batch_size > self.num_samples
        ):
