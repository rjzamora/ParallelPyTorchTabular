...
class PrAucMetric(Metric):
    ...
    def compute(self):
        ...
        print(
            "Number of targets for {}-Curve: {}".format(
        ...
        if not self._ignore_bad_metric and len(x) == 2:
            raise MetricCurveError(
                "{}-Curve returned only two points!".format(self.name)
            )
        start = time.time()
        output = auc(x, y)
        return output
...
class EarlyStopping(IgniteEarlyStopping):
    ...
    def _load_state(self, update_lr=True):
        ...
        new_lr = max(
            self.optimizer.param_groups[0]["lr"] * self.lr_multiplier,
            self.min_lr,
        )
        ...
        if update_lr:
            ...
            self._logger.info(
                "Updated optimizer: {}".format(str(self.optimizer))
            )
    ...
    def __call__(self, engine):
        ...
        if self.best_score is None:
            ...
            self.counter += 1
            print(
                "Score did not improve! EarlyStopping: %i / %i"
...
def run_training(
    ...
    evaluator = create_supervised_evaluator(
        model,
        metrics={"pr-auc": PrAucMetric(ignore_bad_metric=True)},
        device=device,
    )
    ...
    def log_training_loss(engine):
        ...
        )  # (engine.state.iteration - 1) % num_epoch_batches + 1
        if iter % log_interval == 0:
            ...
            epoch_examples_per_second = (
                examples - (engine.state.epoch - 1) * examples_per_epoch
            ) / epoch_time_elapsed
            print(
